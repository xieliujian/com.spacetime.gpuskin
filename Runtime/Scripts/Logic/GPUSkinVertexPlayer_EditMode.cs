using System.Collections;
using System.Collections.Generic;
using UnityEngine;


namespace ST.GPUSkin
{
    /// <summary>
    /// 
    /// </summary>
    public partial class GPUSkinVertexPlayer
    {

#if !ST_GAME_MODE

        /// <summary>
        /// 
        /// </summary>
        /// <param name="mf"></param>
        /// <param name="renderer"></param>
        protected override void OnEditorValidate(MeshFilter mf, MeshRenderer renderer)
        {
            if (infoDB == null)
                return;

            InitMaterial(renderer);
            InitAnimationNameList();
            PlayAnimValidate();
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="renderer"></param>
        /// <param name="currentInfo"></param>
        /// <param name="block"></param>
        protected override void OnUpdateSkin(MeshRenderer renderer, GPUSkinInfoDB currentInfo, MaterialPropertyBlock block)
        {
            OnSetPropertyBlock(renderer, currentInfo, block);
        }

        /// <summary>
        /// 
        /// </summary>
        void PlayAnimValidate()
        {
            var infoList = infoDB.infoList;
            if (infoList == null)
            {
                Play(-1);
                return;
            }

            int infoNum = infoList.Length;
            if (m_AnimationIndex >= infoNum)
            {
                Play(infoNum - 1);
            }
            else if (m_AnimationIndex < 0)
            {
                Play(0);
            }
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="renderer"></param>
        void InitMaterial(MeshRenderer renderer)
        {
            Material mat = renderer.sharedMaterial;
            if (mat == null)
                return;

            mat.SetTexture(GPUSkinDefine.s_GpuSkin_Shader_Vertex_AnimTex, infoDB.texture);
            mat.SetFloat(GPUSkinDefine.s_GpuSkin_Shader_Vertex_AnimTexWidth, infoDB.texWidth);
            mat.SetFloat(GPUSkinDefine.s_GpuSkin_Shader_Vertex_AnimTexHeight, infoDB.texHeight);
            mat.SetFloat(GPUSkinDefine.s_GpuSkin_Shader_Vertex_Count, infoDB.vertexCount);
        }
#endif
    }
}
